...
\frame{
  ...
  \item[{\tt -O2}] Optimize even more.
  \item[{\tt -O3}] Optimize yet more.
  \item[{\tt -Ofast}] Enables optimizations that are not valid for all
    standard-compliant programs. It turns on {\tt -ffast-math} and the
    Fortran-specific {\tt -fstack-arrays}, unless {\tt
      -fmax-stack-var-size} is specified, and {\tt
      -fno-protect-parens}.
  \end{description}

  -> {\footnotesize Check the specific options what are being
    enable/disable at each lever for your compiler version in the doc:}
  {\scriptsize
    \url{http://gcc.gnu.org/onlinedocs/gcc/Optimize-Options.html\#Optimize-Options}}
  {\scriptsize \url{http://gcc.gnu.org/onlinedocs}}
  {\tiny (ft2's highest)}
  {\scriptsize \url{http://gcc.gnu.org/onlinedocs/7.2.0}}
}
