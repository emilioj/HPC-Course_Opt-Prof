...
\begin{frame}[fragile]{Obtaining a visual output: {\tt Gprof} and {\tt
  ...
  \begin{block}{{\tt gprof2dot}}
    \begin{itemize}
    \item[$\Rightarrow$] Python script to convert profiler output into a dot graph
    \end{itemize}
  \end{block}
...
\begin{frame}[fragile]{Valgrind: Memcheck example}
  ...
  \vspace*{-0.8cm}%
  \begin{lstlisting}[style=c,gobble=1,caption={C code with some memory
    issues}]
    #include <stdlib.h>
